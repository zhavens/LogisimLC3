...
import type ErrorBuilder from "./errorBuilder.js";
...
export default class Parser
{
        ...
        bits: number = 16,) : number
    {
        ...
        if (isNaN(result))
        {
            console.error(this.errorBuilder.immOperand(lineNum, token) + "\n");
            return NaN;
        }
        // value does not fit in allowed bits
        else if (result < min || result > max)
        {
            console.error(this.errorBuilder.immBoundsBits(lineNum, bits, token) + "\n");
            return NaN;
        }
        // parsing successful
        else
    ...
    protected stringToCodes(literal: string, lineNum: number) : number[] | null
    {
        ...
        {
            console.error(this.errorBuilder.badQuotes(lineNum, literal) + "\n");
            return null;
        }
        return result;
    }
    ...
    protected parseReg(regStr: string, lineNum: number) : number
    {
        ...
        {
            console.error(this.errorBuilder.badRegister(lineNum, regStr) + "\n");
            return NaN;
        }
        else
        {
    ...
    : number
    {
        ...
        if (labels.has(label))
        {
            ...
            {
                console.error(this.errorBuilder.labelBounds(lineNum, label, bits) + "\n");
                return NaN;
            }
            else
            {
        ...
        {
            console.error(this.errorBuilder.badLabel(lineNum, label) + "\n");
            return NaN;
        }
    }
    ...
    : number
    {
        ...
        {
            case ".orig":
                console.error(this.errorBuilder.multiOrig(lineNum) + "\n");
                break;
            ...
            case ".stringz":
                ...
                {
                    console.error(this.errorBuilder.emptyString(lineNum) + "\n");
                }
                break;
        }
