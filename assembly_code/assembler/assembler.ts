 ...
 * 
 * Originally from the WebLC3 simulator by Aiden Garvey, with minor adjustments for CLI operation:
 *      https://github.com/aidan-garvey/WebLC3
 */
...
export default class Assembler
{
        ...
        : Promise<[Uint16Array, Map<number, string>] | null>
    {
        ...
        {
            console.error(this.errors.INFILE + "\n");
            return null;
        }
        ...
        {
            console.error(this.errors.FIRSTLINE + "\n");
            return null;
        }
        else
        {
            ...
            {
                console.error(errorBuilder.operandCount(lineNum, tokens) + "\n");
                return null;
            }
        ...
        while (!atEnd && ++lineNum < srcLines.length)
        {
            ...
            if (currLine)
            {
                ...
                {
                    console.error(errorBuilder.unknownMnemonic(lineNum, tokens[0]) + "\n");
                    hasError = true;
                }
            } // end if
        } // end white
        ...
        for (const entry of toFix)
        {
            ...
            {
                console.error(errorBuilder.noLineNumForAddr(loc) + "\n");
                lineNum = -1;
            }
            else
            {
            ...
            if (tokens[0] == ".fill")
            {
                ...
                {
                    hasError = true;
                    console.error(errorBuilder.badLabel(lineNum, tokens[1]) + "\n");
                }
                else
                {
            ...
            else if (tokens[0] == ".blkw")
            {
                if (labels.has(tokens[2]))
                {
                    ...
                    {
                        hasError = true;
                        console.error(errorBuilder.badLabel(lineNum, tokens[2]) + "\n");
                    }
                }
                else
                {
            ...
            else
            {
                ...
                {
                    hasError = true;
                    console.error(errorBuilder.badLabel(lineNum, tokens[0]) + "\n");
                }
            }
        }
        ...
        for (let i = 0; i < memory.length; i++)
        {
            ...
            if (memory[i] > 0xFFFF)
            {
                console.error(errorBuilder.badMemory(lastLineNum, i + startOffset, memory[i]) + "\n");
                hasError = true;
                result[i + 1] = 0;
            }
            else if (isNaN(memory[i]))
            {
                console.error(errorBuilder.nanMemory(lastLineNum, i + startOffset) + "\n");
                hasError = true;
                result[i + 1] = 0;
            }
            else
...
function runAssembler() {
    if (process.argv.length <= 2) {
        console.error("Missing argument: [asm_file]");
        return;
    }
    ...
    fs.readFile(process.argv[2], 'utf8', async (err, data) => {
        ...
        Assembler.assemble(data, true);
        let blob = Assembler.getObjectFileBlob(); 
        if (blob) {
            let objpath = filepath.replace(".asm", ".obj");
            let buffer = await blob.bytes();
            fs.writeFile(objpath, buffer, {}, (err) => {
                if (err) {
                    console.error(err);
                    return;
                    } else {
                    console.log("Wrote output to " + objpath);
                    }
            });
        }
        });
